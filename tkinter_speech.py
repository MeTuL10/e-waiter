...
say("WELCOME!! would you like to continue in our handsfree mode of ordering if interested.... say yes!! else say no")
with mic as source:                                                                                                                                                         
    audio = r.listen(source)   
mode=r.recognize_google(audio)
print("You said : {}".format(mode))
if mode=="yes":
    bill=0

    menustr='1:INDIAN APPETIZERS \n2:ITALIAN APPETIZERS \n3:INDIAN MAIN COURSE\n4:INDIAN BREADS AND RICE\n5:PIZZAS\
                n6:SANDWICHES\n7:SALADS\n8:INDIAN DESSERTS\n9:ITALIAN DESSERTS'
    menu={1:'INDIAN APPETIZERS',2:'ITALIAN APPETIZERS',3:'INDIAN MAIN COURSE',
         4:'INDIAN BREADS AND RICE',5:'PIZZAS',6:'SANDWICHES',7:'SALADS',
        8:'INDIAN DESSERTS',9:'ITALIAN DESSERTS'}

    indapt= {1:['Assorted Non-veg Platter',399],2:['Chicken Chaat',350],3:['Shrimp Pakore',250],4:['Shammi Kebab',450],
             5:['Chicken Pakore',399],6:['Hera Bhera Kebab',199],7:['Onion Bhaji',299],8:['Vegetable Tikka',199],
             9:['Samose',100]}
    italapt  = {1:['SEASONAL SOUP ',199],2:['GRILLED ARTICHOKE ',350],3:['PROSCIUTTO BRUSCHETTA ',250],
                4:['ITALIAN MEATBALLS ',450],5:['CHEFS BOARD',399],6:['GRILLED CAULIFLOWER',199],7:['CRISPY CALAMARI ',299],
                8:['WHITE TRUFFLE GARLIC BREAD ',199],9:['ZUCCA CHIPS',100],10:['BURRATA ',200],11:['ARANCINI ',200]}
    indmain  = {1:['Palace Handi',129],2:['Murg Chakori',189],3:['Mango Chicken',200],4:['Murg Tikka Lajawab',200],
                5:['Bombay Shrimp Curry',199], 6:['Achari Kadhai',199],7:['Vindaloo',299],8:['Madras Curry',199]}
    indb     = {1:['Vegetable Biryani',399],2:['Shrimp Biryani',399],3:['Hydrabadi Beef Biryani',
               300],4:['Gosht Lamb Biryani',399],5:['Shahi Chicken Biryani',399],6:['Bread Basket',
                299],7:['TanDoori Roti',199]}
    piz      = {1:['daily pizza',299],2:['chef\'s choice',299],3:['margherita',199],
                4:['prosciutto',199],5:['funghi',199],6:['bacon & egg',299],
                7:['the pig',199]}
    sdw      = {1:['grilled chicken',399],2:['italian grinder',299],3:['our famous meatball sandwich',299],
                4:['the burger',199]}
    sal      = {1:['Caeser',299],2:['Arugula & Roasted fennel',199],3:['Tuscan kale',189],4:['Seasonal Vegetable',150],
                5:['chopped Chicken',300],6:['Italian Farm',200]}
    des      = {1:['Palace Mango Sundae',199],2:['Mango Triffle',159],3:['Mango Kulfi',89],4:['Pista Kulfi',60],
                5:['Gajar Halwa',100],6:['Gulab Jamun',100],7:['Shahi Rasmalai',100],8:['Badamee Kheer',150]}
    italdes  ={ 1:['Biscotti',299],2:['Pannacota',299],3:['Gelato',89],4:['Choco pie',50],5:['Apple Pie',400],
                6:['Tiramissu',90]}
    say("To begin with the order say yes")
    with mic as source:                                                                                                                                                         
        audio = r.listen(source)   
    ...
    while re=='yes':
        ...
        if text=="Indian appetizers":
            n=indapt
            say(n)
        if text=="Italian appetizers":
            n=italapt
            say(n)
        if text=="Indian main course":
            n=indmain
            say(n)
        if text=="Indian breads and rice":
            n=indb
            say(n)
        if text=="pizzas":
            n=piz
        ...
        if text=="sandwiches":
            n=sdw
            say(n)
        if text=="salads" or text=="salad":
            n=sal
            say(n)
        if text=="Indian desserts":
            n=des
            say(n)
        if text=="Italian desserts":
            n=italdes
            say(n)
        say('please tell us the dish number u want to order')
        with mic as source:                                                                                                                                                         
            audio = r.listen(source)   
        dish=r.recognize_google(audio)
        print("You said : {}".format(dish))
        say(n[int(dish)])
        bill+=n[int(dish)]
        bill+=n[int(dish)][1]
        bill+=(bill*18/100)+(bill*10/100)
        say("to cancel any items say edit")
        say("to go back to main menu say yes")
        say("to exit and generate bill say exit")
    with mic as source:                                                                                                                                                         
        audio = r.listen(source)   
    re=r.recognize_google(audio)
    print("You said : {}".format(re))
    if re=='yes':
        ...
        fbill=("the cost inclusive of taxes is",bill)
        say(bill)
    elif re=="cancel":
        say(bill)
        say("please tell the name of the dish u would like to remove")
        with mic as source:                                                                                                                                                         
            audio = r.listen(source)   
        ca=r.recognize_google(audio)
        print("You said : {}".format(ca))
        bill-=n[int(ca)][1]
        say("to go back to main menu say yes")
        say("to exit and generate bill say exit")
        with mic as source:                                                                                                                                                         
